...
public class TaskListDaoTestSuite {
    ...
    public void testFindByListName() {
        //given
        TaskList taskList = new TaskList("ToDoTasks", "List of task to be done.");
        taskListDao.save(taskList);
        String listName = taskList.getListName();

        //when
        List<TaskList> savedList = taskListDao.findByListName(listName);

        //then
        Assert.assertEquals(1, savedList.size());

        //cleanUp
        int id = savedList.get(0).getId();
